...
| **Persistent object** | Server-hosted; one persistent host per woah object. Identifier prefix `#`. |
...
- [rest.md](spec/protocol/rest.md) — HTTP+SSE REST API; six endpoints; `$me` (R1–R11) — **retired** (classic `/api/*` transport removed in the 2026-07 net-only cutover)
...
- [v2-turn-network.md](spec/protocol/v2-turn-network.md) — v2 node-network protocol: deterministic VM turns, commit scopes, execution capability gossip, state transfer, live events, in-browser nodes, and scope-local scheduled turns (VTN1-VTN18) — **retired** (the v2 `/v2/*` transport, `CommitScopeDO`, and browser node were removed in the 2026-07 net-only cutover; the ratified transcript/commit semantics are carried forward by coherence.md and reused by the Net stack)
...
Tools and in-world interfaces for making woah programmable by people and agents.
...
- Catalog-owned design docs for bundled catalogs (see [spec/discovery/catalogs.md §CT15](spec/discovery/catalogs.md#ct15-bundled-catalogs-in-this-repo) for the role of each):
  ...
  - **Demo applications** — [dubspace](catalogs/dubspace/DESIGN.md), [tasks](catalogs/tasks/DESIGN.md), [pinboard](catalogs/pinboard/DESIGN.md), [outliner](catalogs/outliner/DESIGN.md).
