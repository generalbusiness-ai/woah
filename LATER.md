...
- cross-host session-record propagation. Each host DO has its own `world.sessions` map; the cross-host call envelope carries `ctx.session` (the id) but not `session.currentLocation` or the rest of the session record. Two visible symptoms: (1) `:enter` on a feature-space or remote room emits `origin: "$nowhere"` because the dest host's `currentLocationForSession(ctx.session)` reads either a stale or default-shaped local session record; (2) cross-host `:leave` returns `this.subscribers` still containing the leaver because `updatePresenceChecked` defers the actor-presence side via `deferHostEffect` while reading subscribers locally before reconciliation. Fix shape: include `session_current_location` (and likely `session_actor`) in `serializedCallContext`; on the receive side, upsert the dest host's session record before dispatching. Repro: walk chatroom → deck → enter hot_tub (note `origin: "$nowhere"` despite prior location = the_deck), then `:leave` and observe returned `subscribers` still includes the leaver.
