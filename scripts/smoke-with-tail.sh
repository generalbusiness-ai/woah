...
# per-host timing and Net integrity analysis. Tail JSON is captured to a
# tempfile; the analyzers read it and print aggregated stats. The smoke's own
# pass/fail summary follows the analysis.
#
# Usage:
...
NET_ANALYSIS="${NET_ANALYSIS:-$OUT_DIR/net-metrics-report.txt}"
ANALYZER="scripts/analyze-smoke-tail.mjs"
NET_ANALYZER="scripts/net-metrics-report.ts"
...
echo ""
npx --no-install tsx "$NET_ANALYZER" "$TAIL_LOG" --min-turns 20 --diagnostic | tee "$NET_ANALYSIS"
net_status=${PIPESTATUS[0]}
set -e
...
echo "smoke-with-tail: Net metrics analysis saved to $NET_ANALYSIS"
...
if [[ "$failures" -gt 0 || "$tail_status" -ne 0 || "$time_status" -ne 0 || "$net_status" -ne 0 ]]; then
  exit 1
fi
