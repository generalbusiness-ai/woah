...
**Remote command planning reads.** A room-hosted command planner may need to inspect a visible object's verb metadata when that object is hosted elsewhere. Command planning must ask the owning host for all verb candidates whose canonical name or aliases match the command token, not just the first runtime-resolved verb: command aliases intentionally overlap, such as `look` and `look_at`, and the planner filters candidates by `arg_spec.command`. The host RPC returns the definer, slot, canonical verb name, aliases/arg spec needed for planning, and `direct_callable` flag. The resulting command plan carries `(verb_definer, verb)`; execution verifies that page remains reachable from the target and re-checks permissions on the object host instead of re-resolving the name from the receiver.
...
| `commandVerbCandidates(id, name)` | Read-only command-planning metadata lookup. Returns every local ancestry/feature verb whose canonical name or aliases match `name`, preserving local planner order, with definer, slot, `arg_spec` (including command metadata), and `direct_callable`. Command planning filters this list by `arg_spec.command` and binds the selected definer into the plan; execution verifies and invokes that exact page. |
...
refresh and not a global sweep. The cold-load repair is tracked by its own
resident derived-contents repair epoch, separate from the bundled catalog
fingerprint, so a code-only repair deploy can run exactly once on already-current
catalog hosts. A wizard may run the same repair on already-live
...
RECEIVE-side twin: one event per inbound `/net/fanout` request (single row
or lane batch) — INCLUDING failed requests, which stamp `status:"error"`,
the error, and the rows durably applied before the throw (a success-only
emit would bias the occupancy series toward survivors); `ms` spans mirror
