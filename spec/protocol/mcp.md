...
This is the explicit primitive the spec promises in §M2.2 — when an agent calls `the_bug_board:listing()` and gets back ten task summaries, it focuses the task refs it cares about and their per-task verbs (`claim`, `pass`, `release`, `handoff`, `reject`, `wait`, `yield`, `drop_terminal`) join the tool list. Tools alone do not implicitly grow the scope.
