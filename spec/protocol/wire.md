...
//   message   — message map: { actor, target, verb, verb_definer?, verb_slot?, args, body? }
...
- The REST/RPC `result` envelope for a direct call optionally includes `audience_actors: [actor, ...]` (the union audience for all observations), `observation_audiences: [[actor, ...], ...]` (one entry per observation, parallel to `observations`), `audience_sessions: [session, ...]`, `observation_session_audiences: [[session, ...], ...]`, and the parallel `observation_audience_modes: ["presence" | "explicit", ...]`. A sharded relay resolves `presence` against each destination shard's local presence relation; only `explicit` makes the enumerated identities the delivery filter.
- For `presence` mode, a sharded host ignores the incomplete identity enumeration and selects sessions from its local presence rows for the audience scope (including the standard self-suppression rules). For `explicit` mode, it accepts a matching per-observation session or actor hint; scope-filtered presence rows keep another tab for that actor in a different space out. Without the mode vector, rolling-upgrade hosts retain the older session-first order: `observation_session_audiences[i]`, `audience_sessions`, `observation_audiences[i]`, `audience_actors`, then presence fallback.
- Directed observations (`told` and `text`, per events.md §12.7.1) are routed by their explicit recipient fields even when `observation_audiences` is missing — this is the legacy fallback path.
