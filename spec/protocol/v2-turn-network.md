...
`stale_head`, `scope_mismatch`, `incomplete_transcript`, and
`permission_denied` outrank
...
step. Completeness is the terminal step-4 envelope gate and outranks
generation-specific failed-effect admission; both authorities therefore return
`incomplete_transcript` when both defects are present. `nondeterministic` is not
short-circuited, so a post-state-mismatch verdict cannot be silently relabelled.
