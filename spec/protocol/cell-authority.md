...
- **Commit snapshot fallback is also disabled on repair attempts.** A warm MCP
  turn MAY use the scope authority's durable snapshot (the `NetScopeDO` snapshot
  in the current reference) instead of waking every remote owner for the first
  ordinary commit refresh. Once the executor is on a
