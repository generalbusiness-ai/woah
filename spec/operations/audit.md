---
date: 2026-07-16
status: partial — AU1–AU7 implemented on the net stack (records minted in the commit transaction; durable /audit lane; NetAuditDO shards with hash-chained SQLite segments; gateway edge records; /net-api/audit identity-scoped query; AU10 gates 1/2/3/4/5/6 in the curated suite). AU8 is PARTIAL: implemented — sampling decided once at mint and encoded in the W3C flags, gateway `net.turn` root with queue/rpc phase children (root covers queue + wall; children contained), scope `net.commit` (fresh acceptance only; replays/rejections emit `net.scope.submit`), woo.span structured logs plus bounded OTLP/HTTP push (timeout + in-flight cap + counted drops). NOT yet implemented from AU2's normative seam list: gateway plan/submit/repair-attempt subspans, the VM-run span, per-Host.rpc child spans (all need per-call context threading at the TR1 seam — design with TR7.2), and span-LINK emission for async continuations (outbox deliveries, adoption commits, alarm retries, scheduled turns carry their context durably but no producer emits linked spans yet). Remaining draft elsewhere: AU6.3 object-storage segment offload, AU7 push export, AU9 retention/redaction policy machinery, CO17-interplay drain rules
---
