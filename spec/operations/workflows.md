...
- **Time-based transitions.** "After 24h, auto-cancel" is a scheduled turn that calls `:transition` ([semantics/scheduling.md](../semantics/scheduling.md)), armed on entry to the state and cancelled by any transition out of it — use a stable key per item so re-entering the state re-arms rather than accumulating. The deadline is an `always` entry, so it goes through the wizard-owned `$scheduling:deadline` verb. The mechanism is built in, and `catalogs/casework` is the worked example: `open_task` arms an escalation keyed to the task, `claim` cancels it, and `escalate_task` re-checks that the task is still unclaimed before recording `tasks.escalated` — cancellation is best-effort, so a race can still deliver the timer after the work was taken. A workflow-level declarative `timeout` on a state is not built, and remains a refinement.
