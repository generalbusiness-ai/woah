...
- `reindex_ordered_edges` — implemented; derives a room-owned ordered-edge index from a class's legacy `(parent, position)` props. Parameterised by `class`, `parent_prop`, `position_prop`, `edge_prop` (it is generic — it knows nothing of any catalog). For every placed instance of `class` it groups siblings by (container = the instance's `location`, `parent_prop`), orders each group by `(position_prop, then object id)` for a total/reproducible tie-break, and writes `edge_prop = { parent, rank }` with fractional ranks (`src/core/fractional-rank`). Deterministic, so re-deriving from the same input yields byte-identical ranks (idempotent per §CT14.4). It is a synchronous data-transform (no woocode dispatch); pair it with following `drop_property` steps to remove the legacy props. Used by `catalogs/outliner/migration-v1-to-v2.json` (the ordered-edge index — see the outliner `DESIGN.md`).
...
| `dubspace` | **Demo class library** | No | Shared dub-mix sound space. Defines `$dubspace`, `$loop_slot`, `$channel`, `$filter`, `$delay`, `$drum_loop`, `$scene`. Depends on `chat`. **Seeds no instances** — the bundled `the_dubspace` + its child controls live in `demoworld`. |
| `tasks` | **Demo class library + seed** | No | Task-as-obligation-list coordination (see [notes/2026-05-06-task-obligation-model.md](../../notes/2026-05-06-task-obligation-model.md)). Seeds `the_taskboard` as a `$task_registry` without a `location` (no demoworld coupling). Depends on `note`. |
| `pinboard` | **Demo class library** | No | Spatial bulletin board. Defines `$pin`/`$pinboard`/`$kanban_board`. Depends on `chat` + `note`. **Seeds no instances** — the bundled `the_pinboard` lives in `demoworld`. |
| `outliner` | **Demo class library** | No | Shared hierarchical text outliner. Defines `$outline_item`/`$outliner`. Depends on `chat` + `note`. **Seeds no instances** — the bundled `the_outline` lives in `demoworld`. |
| `weather` / `horoscope` | **Demo class library** | Optional | Block primitives (`$weather_block`, `$horoscope_block`). Define their classes only. **Seed no instances** — the bundled `the_weather` and `the_horoscope` live in `demoworld`. |
| `demoworld` | **Demo seed (dependency sink)** | No | The first-light demo. Owns the Living Room / Deck / Hot Tub geography, `$cockatoo`, and every bundled demo instance (`the_dubspace` and its controls, `the_pinboard`, `the_outline`, `the_weather`, `the_horoscope`). Depends on `chat`, `tasks`, `outliner`, `pinboard`, `dubspace`, `weather`, `horoscope`. **Nothing else depends on `demoworld`**, enforced by [`scripts/guard-catalog-layering.mjs`](../../scripts/guard-catalog-layering.mjs). |

**Foundational utilities** are reusable building blocks: a typical operator-deployed world will install some or all four. **Demo class libraries** define classes and verbs but do not seed instances tied to a demo geography; they are reusable in any world. The **demo seed** catalog (`demoworld`) sinks the dependency graph: it depends on every demo class library and is what creates the bundled instances in the bundled rooms.

The dependency direction is enforced one-way: catalogs may depend on `demoworld` only if they themselves are demoworld (i.e. never). The guard rejects any `depends: ["@local:demoworld"]` outside `demoworld`'s own manifest. To ship a new demo instance, the catalog defines its classes and verbs; demoworld adds a `seed_hook` and gains `@local:<new-catalog>` in its own depends.
...
**Class library vs seed catalog distinction.** The split exists for dependency management. A class library defines classes/verbs and may seed instances only when those instances are tied to the class's identity and have no world-specific location (e.g. `tasks` seeds `the_taskboard` without a location, so it doesn't bind to any room). A seed catalog opinionates a populated world; it picks locations, exits, props, and demo placements. `demoworld` is the bundled seed catalog. Every other bundled "demo application" is now a class library; their demo placements live in `demoworld`.
