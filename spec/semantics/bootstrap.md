...
| `:look_at(target)` | obj | The textual command entry for `look <target>`, aliased `look`/`l`/`examine`/`ex` over a closed `dobj: "object"` pattern. Bare `look` is answered by `$root:look`, not here. Dispatches `target:look_self()`, emits private `looked` to the caller, and returns the structured view. Prefers `view.summary` for the observation text so a shape that renders itself reads identically through this entry and through `$root:look`. The `looked.room` field is the command room so clients route the output to the room panel; `looked.target` carries the object actually inspected. |
