...
| `:look_self()` rxd | — | Actor-authored view. Returns the generic title/description plus `carrying`, and appends the current inventory sentence to `description`. It then merges contributions from the actor's worn **features**: verb lookup consults features only after the parent chain, so a feature can never *override* this page by defining `look_self` — it would never be reached. Each feature is asked to describe itself and may only **add** keys; identity and inventory belong to the wearer, so an existing key is never overwritten and the feature object's own `id`/`title`/`carrying` cannot leak in. A feature whose `look_self` raises contributes nothing rather than making its wearer unlookable. This is how the programmer surface reaches a promoted actor. This is woocode seeded on `$actor`, not a substrate special case. |
