...
| `:look_at(target?, target_str?)` | obj, str | The single textual command entry, aliased `look`/`l`/`examine`/`ex`. Its `dobj` pattern is `["object", "none"]`, so one verb serves both the bare `look` and `look <target>` — the shape of `#3:l*ook`, which branches on an empty `dobjstr` in its own body. `target_str` is the raw `dobjstr`: it is what distinguishes *no target given* (bare look, render `this`) from *a target that matched nothing*, which the sentinel alone cannot express. Dispatches `target:look_self()`, emits private `looked` to the caller, and returns the structured view. Prefers `view.summary` for the observation text so a shape that renders itself reads identically through this entry and through `$root:look`. The `looked.room` field is the command room so clients route the output to the room panel; `looked.target` carries the object actually inspected. |
