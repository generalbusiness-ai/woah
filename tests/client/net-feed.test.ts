...
describe("NetFeed turn() over WS", () => {
  it("places the intent on the echo overlay, sends the turn frame, settles on turn_result, and emits self observations", async () => {
    ...
    const turn = feed.turn({ target: "#bob", verb: "wave", verb_definer: "$gesture", verb_slot: 3, args: [], route: "direct" });
    await tick();
    ...
    expect(frame).toMatchObject({ type: "turn", target: "#bob", verb: "wave", verb_definer: "$gesture", verb_slot: 3, args: [] });
...
describe("NetFeed turn() REST fallback", () => {
  it("uses POST /net-api/turn when the socket is down", async () => {
    ...
    feed.onObservation((event) => events.push(event));
    await feed.open();
    const outcome = await feed.turn({ target: "#bob", verb: "wave", verb_definer: "$gesture", verb_slot: 3, args: ["hi"] });
    expect(outcome.status).toBe("accepted");
    const turnCall = calls.find((call) => call.path === "/net-api/turn");
    expect(turnCall?.body).toMatchObject({ target: "#bob", verb: "wave", verb_definer: "$gesture", verb_slot: 3, args: ["hi"], route: "sequenced", session: "s_1" });
