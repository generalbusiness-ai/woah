  ...
  return {
    ...
    close: async () => {
      try {
        ...
        // Teardown must release SQLite even when the bounded quiescence check
        // itself finds a defect.
        fake.close();
      }
    }
  };
...
async function fixture() {
  ...
  return {
    ...
    close: async () => {
      ...
      try {
        // A final assertion can enqueue fanout, and scope alarms can enqueue
        // another deferred pass. Reach the same semantic quiescence as the
        // fixture's normal settle path before closing any SQLite handle.
        await settleAll();
      } catch (err) {
