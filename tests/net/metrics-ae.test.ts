...
describe("net Analytics Engine canary report", () => {
  ...
  it("accepts a distributed, error-free envelope with elastic admission", () => {
    const report: NetAeReport = {
      turns: [
        { host_key: "net-gateway:net-api-0", samples: 300, errors: 0, rpc_timeouts: 0, wall_p99: 240, queue_p99: 120 },
        { host_key: "net-gateway:net-api-1", samples: 300, errors: 0, rpc_timeouts: 0, wall_p99: 310, queue_p99: 180 }
      ],
      authorities: [],
  ...
  it("fails closed on insufficient, concentrated, or divergent evidence", () => {
    const report: NetAeReport = {
      turns: [{ host_key: "net-gateway:net-api-0", samples: 20, errors: 4, rpc_timeouts: 2, wall_p99: 1800, queue_p99: 1400 }],
      authorities: [],
      incidents: [
