...
export class LocalExecutorContext implements ExecutorContext {
  ...
  async commandVerbCandidates(target: ObjRef, verbName: string): Promise<Array<{ name: string; definer: ObjRef; direct_callable: boolean; arg_spec?: Record<string, WooValue> }>> {
  ...
  async dispatch(ctx: CallContext, target: ObjRef, verbName: string, args: WooValue[], startAt?: ObjRef | null, exactDefiner?: ObjRef | null, requireDirectCallable?: boolean): Promise<WooValue> {
    ...
    return await remote.hostDispatch({ ...ctx, world: remote }, target, verbName, args, startAt, chainId, exactDefiner, requireDirectCallable);
  }
