...
describe("shadow lineage commit authority", () => {
  it("accepts create followed by repeated lifecycle replacements in both authorities", async () => {
    ...
    expect(shadowTranscript.writes.filter((write) =>
      write.cell.kind === "lifecycle" && write.cell.object === created.object && write.op === "set"
    )).toEqual([
    ...
    for (const sentinel of ["created", "present"]) {
      ...
      const validation = validateTranscriptAgainstSerializedWorld(before, rehashTranscript(forged));
      expect(validation.ok, `${sentinel} lifecycle sentinel`).toBe(false);
      expect(validation.errors).toContainEqual(expect.stringContaining(
        `read unavailable ${created.object}.lifecycle`
      ));
    }
    ...
    expect(transcript.writes.filter((write) =>
      write.cell.kind === "lifecycle" && write.cell.object === netCreated.object && write.op === "set"
    )).toEqual([
      expect.objectContaining({ value: expect.objectContaining({ parent: "lineage_kind_a" }) }),
      expect.objectContaining({ value: expect.objectContaining({ parent: "lineage_kind_b" }) })
    ]);
