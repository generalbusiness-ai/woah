...
export type MetricEvent =
  ...
  | { kind: "storage_flush"; objects: number; properties: number; sessions: number; deleted_sessions: number; counters: boolean; ms: number; rows?: number; top_properties?: Array<[string, number]>; top_objects?: Array<[ObjRef, number]> }
  ...
  | { kind: "storage_direct_write"; what: "object" | "object_delete" | "property" | "property_delete" | "session" | "session_delete" | "counters" | "log_append" | "log_outcome" | "snapshot" | "log_truncate" | "tombstone"; ms: number; rows?: number }
  ...
  | { kind: "storage_full_save"; trigger: "world_persist" | "persist_full_snapshot" | "host_seed_apply"; rows: number; objects: number; properties: number; verbs: number; logs: number; snapshots: number; sessions: number; tombstones: number; ms: number }
  ...
  | { kind: "mcp_envelope_slim_reseed"; scope?: ObjRef; mode: "slim" | "capsule" | "closure" }
...
// handlers, SpaceLogEntry). For that, a direct recursive copy
