...
function seedUniversal(world: WooWorld): void {
  ...
  native(world, "$system", "list_api_keys_for_owner", "list_api_keys_for_owner", "verb :list_api_keys_for_owner(actor) rxd { /* native: returns redacted api-key metadata for exactly one explicit actor authority. It never enumerates actor authorities and never derives the target from call-frame topology. */ }", { directCallable: true, perms: "rxd", argSpec: { args: ["actor"] } });
