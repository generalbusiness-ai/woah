...
export default {
  async fetch(request: Request, env: NetOnlyEnv): Promise<Response> {
    ...
    const operator = url.pathname.startsWith("/net-operator/");
    const raw = install || smoke || operator ? request.clone() : request;
    request = sanitizePublicHeaders(request);

    if (url.hostname.toLowerCase() === LANDING_HOST) {
      const landing = handleLandingHost(request, env, url);
    ...
    if (operator) return handleNetOperator(raw, env, url);
...
function handleLandingHost(request: Request, env: NetOnlyEnv, url: URL): Response | Promise<Response> | null {
  if (
  ...
  if (url.pathname === "/woah-og.png" || url.pathname === "/woah-og.svg" || url.pathname.startsWith("/icons/")) {
    if (!env.ASSETS) return missingAssetsResponse();
    return env.ASSETS.fetch(request);
  }
  // Protocol clients (notably MCP and WebSocket upgrades) cannot safely
  // follow a marketing-host redirect. Let every machine/control surface use
  // the normal Net router on this hostname.
  if (
    url.pathname === "/healthz" ||
    url.pathname === "/client-config" ||
    url.pathname === "/mcp" ||
    url.pathname === "/connect" ||
    url.pathname.startsWith("/net-api/") ||
    url.pathname.startsWith("/net-install/") ||
    url.pathname.startsWith("/net-operator/") ||
    url.pathname.startsWith("/net-smoke/") ||
    url.pathname.startsWith("/__internal/") ||
  ...
  return json(
    {
      error: {
        code: "E_NO_ASSETS",
        message: "no SPA bundle deployed; run `npm run build` before `wrangler deploy`"
      }
    },
    503
  );
...
async function handleNetInstall(request: Request, env: NetOnlyEnv, url: URL): Promise<Response> {
  ...
  const allowed =
    ...
    ((request.method === "POST" && (verb === "seed" || verb === "activate" || verb === "repair-relations" || verb === "repair-contents" || verb === "repair-definitions")) || (request.method === "GET" && verb === "head"));
  if (!allowed) {
    return json({ error: { code: "E_INVARG", message: "expected a signed net install probe, scope seed, activate, repair-relations, repair-contents, repair-definitions, or head operation" } }, 404);
  }
  try {
...
/** Exact, internal-signed live-operations allow-list. Credential material is
 * generated by operator tooling; this edge sees only the salted verifier and
 * freshly signs the one authority hop. */
async function handleNetOperator(request: Request, env: NetOnlyEnv, url: URL): Promise<Response> {
  try {
    await verifyInternalRequest(env, request);
  } catch (error) {
  ...
  try {
    ...
    const body = JSON.parse(text) as { authority_scope?: unknown };
    if (typeof body.authority_scope !== "string" || !body.authority_scope) {
      return json({ error: { code: "E_INVARG", message: "credential ensure requires authority_scope" } }, 400);
    }
    const stub = resolveNetDestination(env, `scope:${body.authority_scope}`);
    return await stub.fetch(await signInternalRequest(env, new Request("https://do/net/ensure-credential", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: text
    })));
