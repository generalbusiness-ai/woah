...
export class PersistentObjectDO {
  ...
  private ensureMcpScopeTopologySeed(world: WooWorld, scopes: readonly ObjRef[], occupiedScopes: ReadonlySet<ObjRef>): void {
    // A pre-seeded NEIGHBOR that has since become an OCCUPIED commit scope must
    // be evicted: its lineage-only page has no `exits`/`next_seq`, so leaving it
    // resident would shadow the owner's authoritative row and break a move OUT
    // of that scope. Eviction lets the normal authority path re-materialize it
    // with full dynamic cells; dropping it from seededIds stops the export from
    // stamping it owner-deferring. (When the actor later leaves, a fresh seed
    // re-adds it as a neighbor if reached again.)
    if (this.mcpScopeTopologySeed) {
      for (const occupied of occupiedScopes) {
        if (!this.mcpScopeTopologySeed.seededIds.has(occupied)) continue;
        world.evictTopologySeedObject(occupied);
        this.mcpScopeTopologySeed.seededIds.delete(occupied);
        this.mcpScopeTopologySeed.ownerHostById.delete(occupied);
      }
      if (this.mcpScopeTopologySeed.seededIds.size === 0) this.mcpScopeTopologySeed = null;
    }
    ...
    // Never seed a scope the actor occupies as a lineage-only neighbor: it is a
    // commit scope whose dynamic authority must come from its owner. (It is also
    // excluded from the seed-driven refresh suppression, so its owner row is
    // fetched and used.) A row may be both a neighbor-of-A and occupied-as-B
    // across concurrent sessions on a shard; occupancy wins.
    const seedObjects = seed.objects.filter((row) => !occupiedScopes.has(row.id));
  ...
  private async v2GatewayAuthorityPayload(
    ...
    // way"). Compute the occupied set from the world's session-actor locations
    // plus the reconstruction scope, and exclude it from both the seed and the
    // seed-driven byHost suppression below.
    const occupiedScopes = new Set<ObjRef>();
    if (mcpGatewayShard) {
      if (reconstructionScope && reconstructionScope !== "$nowhere") occupiedScopes.add(reconstructionScope);
      for (const session of world.sessions.values()) {
        const loc = world.objects.get(session.actor)?.location;
        if (loc) occupiedScopes.add(loc);
      }
    }
    ...
    // neighbor rooms (e.g. the_deck reached from the_chatroom's exit) resolve
    ...
    if (mcpGatewayShard) this.ensureMcpScopeTopologySeed(world, requestedIds, occupiedScopes);
    ...
    const local = mcpGatewayShard
      ? mcpGatewayLocalAuthorityPayload(world, requestedIds, localActorAuthorityRoots, this.mcpScopeTopologySeed)
    ...
    const resolvedIds = await Promise.all(requestedIds.map(async (id) => {
      // Suppress the remote refresh only for a pure pre-seeded NEIGHBOR. An id
      // the actor now occupies (the commit scope) is excluded from suppression
      // so its owner-authoritative row (with live exits/next_seq) is fetched and
      // displaces the seeded lineage-only page by CA11 precedence.
      if (seededTopologyIds?.has(id) && world.objects.has(id) && !occupiedScopes.has(id)) return [id, localHost] as const;
    ...
    if (process.env.PROBE_WEST === "1" && requestedIds.includes("the_deck" as ObjRef) && occupiedScopes.has("the_deck" as ObjRef)) {
      // eslint-disable-next-line no-console
      console.error(`PROBE_WEST BYHOST occupied=${JSON.stringify(Array.from(occupiedScopes))} resolved=${JSON.stringify(resolvedIds)} byHost=${JSON.stringify(Array.from(byHost,([h,s])=>[h,Array.from(s)]))} useSnap=${options.useCommitScopeSnapshotForRemoteAuthority} deckResidentInWorld=${world.objects.has("the_deck" as ObjRef)} deckSeeded=${this.mcpScopeTopologySeed?.seededIds.has("the_deck" as ObjRef)}`);
    }
    ...
    if (process.env.PROBE_WEST === "1" && requestedIds.includes("the_deck" as ObjRef) && occupiedScopes.has("the_deck" as ObjRef)) {
      const refs = isAuthorityCellSlice(authority) ? authority.page_refs.filter((r) => r.object === "the_deck") : [];
      // eslint-disable-next-line no-console
      console.error(`PROBE_WEST COMBINED2 reconScope=${reconstructionScope} the_deck pages: ${JSON.stringify(refs.map((r) => ({ page: r.page, name: r.name, source: r.source })))}`);
    }
...
function mcpGatewayLocalAuthorityPayload(
  ...
  for (const id of explicitIds) {
    ...
    //
    // CA11.2 exception: a one-hop neighbor room (e.g. the_deck) whose lineage
...
function mcpGatewayBundledScopeTopologyObjects(): ScopeTopologyClosure {
  ...
  const walkClassChain = (start: ObjRef | null): void => {
    ...
    while (current && !seen.has(current)) {
      ...
      // (a contained scope, e.g. the_hot_tub located in the_deck) is handled by
...
function mcpGatewayScopeTopologySeed(servedScopes: Iterable<ObjRef>): {
